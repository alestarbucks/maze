...
class Maze:
    def __init__(self, height, width, exits=0, verbose=0):
        if height % 2 == 0:
            if verbose > 0: print("Warning: x_size must be odd. Adding 1.")
            self.x_size = height + 1
        else: self.x_size = height

        if width % 2 == 0:
            if verbose > 0: print("Warning: y_size must be odd. Adding 1.")
            self.y_size = width + 1
    ...
    def save_image(self, path, id="", scale=1):
        # plt.axis('off')
        # plt.imsave(path+'/maze{}.png'.format(id),self.grid,cmap='gray',dpi=200)
        im = Image.fromarray(np.array(self.grid, dtype='uint8') * 255)
        if scale != 1:
            im = im.resize((self.x_size * scale, self.y_size * scale), Image.NEAREST)
        output_path = path+'/maze{}.png'.format(id)
        im.save(output_path)
